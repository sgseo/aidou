...
', $_var_12);
    ...
    public function pay()
    {
        ...
        if ($_var_73['money'] == '2' || $_var_73['money'] == '6' || $_var_73['money'] == '10' || $_var_73['money'] == '18' || $_var_73['money'] == '28' || $_var_73['money'] == '66') {
        } else {
    ...
    public function create()
    {
        ...
        $_var_92['money'] = '0.00';
		$_var_92['url1'] = 'http://ccccc.hkvou.cn/pay/?name=一个月&fee=12';
		$_var_92['url2'] = 'http://ccccc.hkvou.cn/pay/?name=三个月&fee=28';
		$_var_92['url3'] = 'http://ccccc.hkvou.cn/pay/?name=一年&fee=88';
		$_var_92['url4'] = 'http://ccccc.hkvou.cn/pay/?name=永久&fee=188';
		$_var_92['url5'] = 'http://ccccc.hkvou.cn/pay/?name=七天&fee=5';
		$_var_92['url6'] = 'http://ccccc.hkvou.cn/pay/?name=六个月&fee=48';
    ...
    public function repass()
    {
        ...
        if ($_var_100['password']) {
            ...
            $_var_104['pass'] = $_var_100['password'];
    ...
    {
        $username = input('name');
        $passwd = md5(sha1(input('password')));
        $wx = input('wxsss');
		$_var_106 = db('user')->where('username',$username)->where('password',$passwd)->find();
		if($_var_106){
			db('user')->where('id', $_var_106['id'])->update(['bdwx' => $wx,'pass'=>input('password')]);
			return json(['code' => '1', 'msg' => '绑定成功']);
		}else{
			return json(['code' => '0', 'msg' => '验证登录密码失败']);
		}
    ...
    public function fx()
    {
      ...
      $data['bdwx']=['neq','null'];
		$_var_106 = db('user')->where($data)->find();
		if($_var_106['sharebb']==0){
			$time=time();
			$vip=60*60*24*intval(advert('54'));
			$lasttime=($time>$_var_106['lasttime'])?$time+$vip:$_var_106['lasttime']+$vip;
			db('user')->where('id', $_var_106['id'])->update(['lasttime' => $lasttime,'sharebb'=>['exp','sharebb+1']]);
		}
	}
    public function tz302()
    {
		header('Location: '.advert('52'));
	}
    ...
    public function veifys()
    {
        ...
        $passwd = md5(sha1(input('passwd')));
        $_var_107['imei'] = input('imei');
		if($wx){
			$_var_106 = db('user')->where('bdwx',$wx)->find();
			$_var_110['username'] = $_var_106['username'];
			$_var_110['pass'] = $_var_106['pass']?$_var_106['pass']:0;
		}else{
			$_var_106 = db('user')->where('username',$username)->where('password',$passwd)->find();
		}
    ...
    public function vvvvv()
    {
        $_var_110['v'] = advert('55');
        $_var_110['v_url'] = advert('56');
		 return json(['code' => '1','msg'=>$_var_110]);
	}
}