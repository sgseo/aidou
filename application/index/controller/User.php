...
class User extends Controller
{
    ...
    public function clog()
  {
        $xzv_26['time'] = ['neq',0];
      	if(isset($_GET['id'])){
          $xzv_47 = db('user')->where('username like "%' . input('id') . '%"')->column('id');
          $xzv_26['uid'] = implode(',', $xzv_47);
        }
        $xzv_50 = db('ypay')->where($xzv_26)->order('time', 'desc')->paginate(10, false, ['query' => input()]);
